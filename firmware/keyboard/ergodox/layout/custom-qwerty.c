...
// ----------------------------------------------------------------------------
// Custom wtf it is stuff
// 
// I have lights wired up to pin C7 on the teensy, so that's a thing.
// Setting that pin high/low turns them on off.
// ----------------------------------------------------------------------------

#define custom__ambient_lights_right_on()       \
    DDRC |= (1<<7)

#define custom__ambient_lights_right_off()      \
    DDRC &=~ (1<<7)

#define custom__ambient_lights_read()           \
    DDRC & (1<<7)

void custom__ambient_lights_toggle(void) {
    if (custom__ambient_lights_read()) { // true: light is on
        custom__ambient_lights_right_off();
    }
    else {
        custom__ambient_lights_right_on();
    }
}

#define P(ambTgl) custom__ambient_lights_toggle
#define R(ambTgl) KF(nop)
